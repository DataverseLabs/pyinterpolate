...
class ArealSemivariance:
    ...
    def __init__(self, areal_data_file, population_data_file, lags=None, step_size=None, id_filed='ID'):
        """Class for calculation of Areal semivariance. It us used by the methods of Area-to-Area and Area-to-Point
           ...
           population_data: vector files with population blocks for each area.
           To maintain stability areas in the areal_data and population blocks in the population_data must have
           the same ID filed - id_filed, which allows algorithm to merge those datasets.
    ...
    def centroids_semivariance(self, lags=None, step_size=None, update=True, data_column='DATA'):
        ...
        # Calculate semivariances - this fragment must be transferred to the private method !!!
        for h in lags:
            gammas = []
        ...
        if update:
            ...
            self.point_support_semivariance = semivariance.T
    ...
    def _centroids_from_shp(self, data_column):
        ...
        gdf[col_x] = gdf['geometry'].apply(lambda x: x.centroid.x)
        gdf[col_y] = gdf['geometry'].apply(lambda x: x.centroid.y)
        
        columns_to_hold = [col_x, col_y, data_column]
        ...
        centroids_and_vals = np.asarray(gdf.values)
