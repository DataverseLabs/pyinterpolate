...
def calculate_semivariogram_deviation(data_based_semivariogram, theoretically_regularized_semivariogram):
    ...
    if array_length == len(theoretically_regularized_semivariogram):
        if (len(data_based_semivariogram) == len(theoretically_regularized_semivariogram[:, 1])):
            print('Start of deviation calculation')
            deviation = np.abs(theoretically_regularized_semivariogram[:, 1] - data_based_semivariogram)
            deviation = np.nan_to_num(deviation / data_based_semivariogram)
...
class DeconvolutedModel:
    ...
    def regularize(self, areal_data_file, areal_lags, areal_step_size, data_column,
        ...
        self.centroids = centroids_semivar.centroids.copy()

        theoretical_model = TheoreticalSemivariogram(self.centroids,
        ...
        # Deviation
        deviation = calculate_semivariogram_deviation(self.base_point_support, regularized)
        self.optimal_difference_statistics = deviation
        self.append_information()

        # Model fitting loop
        while self.iteration_number < 3:
            ...
            print('Testing deviation parameters')

            if deviation < self.optimal_difference_statistics:
                self.experimental_point_support = regularized
                self.change_weight = False
                self.optimal_difference_statistics = deviation
            else:
        ...
        return self.optimal_regularized_model
    ...
    def rescale(self):
        ...
        y_opt = self.experimental_point_support
        y_v_exp = self.base_point_support
        s = self.sill_of_areal_data**2
        y_v_opt = self.optimal_regularized_model
        i = self.iteration_number

        if not self.change_weight:
            w = 1 + (y_v_exp - y_v_opt[:, 1]) / (s * np.sqrt(i))
        else:
            w = 1 + (self.weights - 1) / 2

        self.weights = w.copy()
        rescalled = y_opt.copy()
        rescalled[:, 1] = y_opt[:, 1] * w
        self.temp_point_support_model = rescalled

        return rescalled


    def append_information(self):

        self.deviations.append(self.optimal_difference_statistics)
        self.regularized.append(self.optimal_regularized_model)
        print('List of deviations and regularized updated')
        return 0
