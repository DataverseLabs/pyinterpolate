...
class AggregatedVariogram:
    ...
    agg_lags : numpy array
        Lags calculated as a set of equidistant points from ``step_size`` to ``max_range`` with a step of size
        ``step_size``.
    ...
    def __init__(self,
        ...
        self.nugget = nugget
        self.agg_lags = np.arange(self.step_size, self.max_range, self.step_size)
    ...
    def regularize(self,
                   ...
                   theoretical_block_model: TheoreticalVariogram = None) -> np.ndarray:
        r"""
    ...
    def show_semivariograms(self):
        ...
        if self.regularized_variogram is None:
            raise AttributeError('Variograms may be plot after regularization process. Use regularize() dir_neighbors_selection_method.')
        else:
    ...
    def _fit_theoretical_model(self) -> TheoreticalVariogram:
        ...
        parameter in regularize() dir_neighbors_selection_method.
...
def regularize(blocks: Blocks,
    ...
    agg_var = AggregatedVariogram(blocks=blocks,
