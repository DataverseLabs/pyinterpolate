...
def circular_model(lags: np.ndarray,
                   nugget: float,
                   sill: float,
    ...
    (1) $\gamma = c0 + c[1 - (\frac{2}{\pi} * arccos(a)) + (\frac{2}{\pi} * a)
        * \sqrt{1 - a^{2}}]$, $0 < a <= h$;
    (2) $\gamma = c0 + c$, $a > h$;
    (3) $\gamma = 0$, $a = 0$.
    ...
    Bibliography
    ------------
    [1] McBratney, A. B., Webster R. Choosing Functions for Semivariograms of
    Soil Properties and Fitting Them to Sampling Estimates. Journal of Soil
    ...
    arp = ar[poses]
    ar2 = np.power(arp, 2)
    gamma[poses] = nugget + sill * (
            1 - (pic * np.arccos(arp)) + (pic * arp) * np.sqrt(1 - ar2)
    )
...
def cubic_model(lags: np.ndarray,
                nugget: float,
                sill: float,
                rang: float) -> np.ndarray:
    ...
    (1) $\gamma = c0 + c *
         (7 * (\frac{a}{h})^{2} - 8.75 * (\frac{a}{h})^{3} +
         3.5 * (\frac{a}{h})^{5} - 0.75 * (\frac{a}{h})^{7})$,
         $0 < a <= h$;, $0 < a <= h$;
    (2) $\gamma = c0 + c$, $a > h$;
    (3) $\gamma = 0$, $a = 0$.
    ...
    Bibliography
    ------------
    [1] Armstrong, M. Basic Linear Geostatistics. ISBN: 978-3-642-58727-6.
        Springer 1998.
...
def exponential_model(lags: np.ndarray,
                      nugget: float,
                      sill: float,
                      rang: float) -> np.ndarray:
...
def gaussian_model(lags: np.ndarray,
                   nugget: float,
                   sill: float,
                   rang: float) -> np.ndarray:
...
def linear_model(lags: np.ndarray,
                 nugget: float,
                 sill: float,
                 rang: float) -> np.ndarray:
...
def power_model(lags: np.ndarray,
                nugget: float,
                sill: float,
                rang: float) -> np.ndarray:
...
def spherical_model(lags: np.ndarray,
                    nugget: float,
                    sill: float,
                    rang: float) -> np.ndarray:
    ...
    (1) $\gamma = c0 + c * ( \frac{3}{2} * \frac{a}{h} - 0.5 *
        (\frac{a}{h})^{3} )$, $0 < a <= h$;
    (2) $\gamma = c0 + c$, $a > h$;
    (3) $\gamma = 0$, $a = 0$.
