...
def detect_outliers_iqr(dataset: np.ndarray,
                        ...
                        iqr_upper=1.5):
    """
    Function detects outliers in a dataset based on the inter-quartile ranges
    ...
    iqr_lower : float
        How many standard deviations from the 1st quartile down is
        the limit of expected values.

    iqr_upper : float
        How many standard deviations from the 3rd quartile up is
        the limit of expected values.
    ...
    if iqr_upper < 0 or iqr_lower < 0:
        msg = ('Parameters iqr_lower and iqr_upper must be floats greater '
               'or equal to 0.')
...
def detect_outliers_z_score(dataset: np.ndarray,
    ...
    if z_lower >= 0:
        raise ValueError(f'The parameter z_lower must be '
                         f'a float lesser than zero.')
    if z_upper <= 0:
        raise ValueError(f'The parameter z_upper must be '
                         f'a float greater than zero.')
...
def root_mean_squared_error(real_values, predictions) -> float:
    """
    Calculates Root Mean Squared Error of prediction.

    Parameters
    ----------
    real_values : Union[int, float, ArrayLike]

    predictions : Union[int, float, ArrayLike]
    ...
    rmse = np.sqrt((real_values - predictions) ** 2)
    return float(rmse)


def sem_to_cov(semivariances: ArrayLike, sill: float) -> np.ndarray:
    """
...
def select_variogram_thresholds(ds: Iterable,
    ...
    """
    Function selects ``n_thresh`` thresholds of a sample dataset from its
    histogram, it divides histogram based on the n-quantiles.
...
def remove_outliers(data: Union[Iterable, Dict],
    ...
    z_lower_limit : float
        Number of standard deviations from the mean to the left side of
        a distribution. Must be lower than 0.

    z_upper_limit : float
        Number of standard deviations from the mean to the right side of
        a distribution. Must be greater than 0.

    iqr_lower_limit : float
        Number of standard deviations from the 1st quartile into the lowest
        values. Must be greater or equal to zero.

    iqr_upper_limit : float
        Number of standard deviations from the 3rd quartile into the largest
        values. Must be greater or equal to zero.
