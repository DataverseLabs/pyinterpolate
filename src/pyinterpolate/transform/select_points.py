...
from pyinterpolate.distance.angular import calc_angles, calculate_angular_difference
...
def select_kriging_data(unknown_position: ArrayLike,
    ...
    neighbors_range : float
        Range within neighbors are affecting the value, it should be close or the same as the variogram range.

    number_of_neighbors : int, default = 4
        Number of the n-closest neighbors used for interpolation.

    use_all_neighbors_in_range : bool, default = False
        ``True``: if number of neighbors within the ``neighbors_range`` is greater than
        the ``number_of_neighbors`` then use all neighbors in range.
...
def select_possible_neighbors_angular(possible_neighbors: np.ndarray,
    ...
    prepared_data_with_angles = prepared_data[angle_differences <= angular_tolerance]
